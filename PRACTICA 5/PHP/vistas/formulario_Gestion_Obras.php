	...
	if (isset($todasObras)) {
		...
		<?php
	        for ($i = 0; $i < count($todasObras); $i++) {
				...
				$publicacionObra = $modeloObra->getObraPublicada($todasObras[$i]["ID"]); 
	        	...
	        	if (($_SESSION['rol']) == "gestor" || ($_SESSION['rol']) == "superusuario") { ?>
					...
					<?php echo "<div id='editObra" . $i . "' style='display:none' >"; ?>
						...
						<form name="formularioEdicionObras" id="formularioEdicionObras" method="POST" >
						   ...
						   <!-- Campo para modificar la publicación de una obra -->
   			    		   <p>¿La obra está publicada? <input type="text" name="ActualPublicacion" ID="ActualPublicacion" value="<?php echo $publicacionObra ?>" readonly></p>	
						   <p>¿Se va a publicar esta obra?</p>
							<select id="ModPublicarObra" name="ModPublicarObra">    
    ...
    if (($_SESSION['rol']) == "gestor" || ($_SESSION['rol']) == "superusuario") { ?>
		...
		<div id="aniadirObra">
			<form name="aniadirObraForm" method="POST">
			   ...
			   <!-- Lista desplegable para decidir si la obra nueva se publica o no -->
			   
			   <p>¿Se va a publicar esta obra?</p>
				<select id="PublicarObra" name="PublicarObra">    
			       <option value="si" selected="selected">Sí</option>
			       <option value="no">No</option>
			   </select> 
