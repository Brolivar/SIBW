...
class ModeloObras{
    ...
    // Método para buscar obras publicadas o no a través de su descripción y su título
    // Para indicar si queremos que sean obras publicadas o no lo especificamos en el segundo parámetro
    function buscarObrasGoogle ($terminoBusqueda, $publicacion) {

        // Buscamos entre las obras publicadas
        if ($publicacion == "publicadas") {

            // Seleccionamos las obras que contienen la cadena en su descripcion y su título
            $seleccion = "SELECT Titulo, ID FROM obras INNER JOIN obraspublicadas ON obras.ID=obraspublicadas.id_obra 
                WHERE descripcion LIKE '%$terminoBusqueda%' OR titulo LIKE '%$terminoBusqueda%'";
        }
        // Buscamos en todas las obras, ya sean publicadas o no
        else {

            // Seleccionamos las obras que contienen la cadena en su descripcion y su título
            $seleccion = "SELECT Titulo, ID FROM obras WHERE descripcion LIKE '%$terminoBusqueda%' OR titulo LIKE '%$terminoBusqueda%'";
        }

        // Resultado de buscar las obras
        $resultado = mysqli_query($this->conexionBD, $seleccion);

        // Array donde se almacenarán las obras coincidentes
        $lista = array();
    ...
    public function aniadirObra ($tituloNuevaObra, $coleccionNuevaObra, $ImagenNuevaObra, $descripcionNuevaObra, $publicacionNuevaObra) {
        ...
        if ($numFilas == 0) { 
            ...
            // Añadimos la obra a su correspondiente colección
            ...
            // Dependiendo de si se va a publicar o no se inserta o no en la tabla de obras publicadas
            if ($publicacionNuevaObra == 'si' && $id != null) {
                $obraPublicada = "INSERT INTO obraspublicadas(id_obra) VALUES ('$id')";
                $resultadoObraPublicada = mysqli_query ($this->conexionBD, $obraPublicada);   
            }
        }
    ...
    public function borrarObra($idObraBorrar) { 
        ...
        // Si era una obra publicada borramos su referencia de la tabla que registra las obras publicadas
        $borrarObraPublicada = "DELETE FROM obraspublicadas WHERE id_obra='$idObraBorrar'";
        $resultado5 = mysqli_query ($this->conexionBD, $borrarObraPublicada);
   ...
   public function getIDVideos(){
    ...
    //de modificar obra.
     public function modificarObra($idM, $descripcionM, $tituloM, $imagenM, $fecha_publicacionM, $coleccionM, $publicacionM){
        ...
        // Añadimos su ID a la tabla "obraspublicadas" siempre y cuando se haya escogido que se va a publicar
        if ($publicacionM == "si") {

            $nuevaObraPublicada = "INSERT INTO obraspublicadas (id_obra) VALUES ('$idM')";
            $resultado = mysqli_query ($this->conexionBD, $nuevaObraPublicada);   
        }
    ...
    // Función para comprobar si una obra está o no publicada
    public function getObraPublicada($id_obra) {

        // Ejecutamos una consulta extrayendo todo
        $seleccion = "SELECT * FROM obraspublicadas WHERE id_obra='$id_obra'";
        $resultado = mysqli_query ($this->conexionBD, $seleccion);

        // Averiguamos cuántas filas ha devuelto la consulta  
        $numFilas = mysqli_num_rows($resultado); 

        // Si la consulta ha devuelto filas es que sí está publicada
        if ($numFilas > 0) { 
            return "Sí";
        } 

        // Si no devuelve ninguna fila es que no se encuentra en la tabla de obraspublicadas
        else 
            return "No";
    }
}
