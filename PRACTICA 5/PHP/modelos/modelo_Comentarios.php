...
class ModeloComentarios {
    ...
    public function getProhibidas() {
        ...
        $seleccion = 'SELECT palabras FROM  prohibidas';  
        $resultado = mysqli_query ($this->conexionBD, $seleccion);
    ...
    //Funcion que modifica un comentario identificado por una fecha, cambiando ese comentario por $comentariofinal
    public function modificarComentario($fechaFinal, $comentariofinal){
        
        // Ejecutamos una consulta comprobando si el email pasado por parámetro ya se encuentra en la bd
        $seleccion = "UPDATE comentarios SET Comentario='$comentariofinal', FechaYHora='$fechaFinal' WHERE FechaYHora='$fechaFinal'";  
        $resultado = mysqli_query ($this->conexionBD, $seleccion);
    }

    //Funcion que recibiendo un comentario identificado por nombre y fecha, lo borra de la bd
    public function borrarComentario($nombre, $comentario, $fecha) {

        // Eliminamos la fila correspondiente al usuario
        $eliminarComentario = "DELETE FROM comentarios WHERE comentario='$comentario' AND FechaYHora='$fecha' AND nombre='$nombre'";
        mysqli_query($this->conexionBD, $eliminarComentario);
    }

    public function buscarComentario($comentarioBuscado){

        //Seleccionamos los comentarios que contienen la cadena

        $seleccion = "SELECT Nombre, FechaYHora, Comentario FROM comentarios WHERE Comentario LIKE  '%".$comentarioBuscado."%'  ";
        $resultado = mysqli_query($this->conexionBD, $seleccion);
