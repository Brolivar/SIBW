...
// Función para buscar obras al estilo Google a través de su título o su descripción
function buscarPaginasEstiloGoogle (terminoBuscado) {

    // Solo realizamos la búsqueda si se ha escrito algo en el cajetín
    if (terminoBuscado.length == 0) { 
        document.getElementById("paginasEncontradas").innerHTML = "";
        document.getElementById("paginasEncontradas").style.border="0px";
        return;
    }

    // Creamos el objeto que nos permitirá enviar una petición al servidor
    var xhttp = new XMLHttpRequest();

    // Función que va modificando el campo donde se mostrarán las obras que coincidan
    // con la búsqueda
    xhttp.onreadystatechange = function() {

        // Solo se modificará el campo de las obras encontradas si la petición ha finalizado y la respuesta
        // está prearada (4) y si el estado de la consulta es OK (200)
        if (this.readyState == 4 && this.status == 200) {
            document.getElementById("paginasEncontradas").innerHTML = xhttp.responseText;
            document.getElementById("paginasEncontradas").style.border="1px solid lightseagreen";
        }
    };

    // Especificamos la petición que le vamos a hacer al servidor. En nuestro caso lo haremos a través de "GET" y 
    // especificándole el fichero en el servidor donde tendrá que encontrar las obras que casen con la búsqueda
    // true para que no se cambie el contenido de los resultados hasta que el servidor no nos devuelva la respuesta (petición asíncrona)
    xhttp.open("GET", "vistas/buscadorPaginas.php?busqueda="+terminoBuscado, true);
